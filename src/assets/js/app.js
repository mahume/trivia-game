let questionsArr = [
    ...
    },
    {
        ...
        possibleAnswers: ["The Grey Pilgrim", "Incanus", "Gandalf the Grey", "Mithrandir"],
    ...
    {
        question: "What changed Arwen's mind about becoming mortal to be with Aragorn?",
        possibleAnswers: 
        [
...
let randomNum, questionTimer, questionResultsTimer, finalResultsTimer, newForm, percentageCalculated; 
let correctTally = 0;
let incorrectTally = 0;
let questionElement = document.querySelector('#questionArea');
let answerElement = document.querySelector('#answersArea');
let questionsAnsweredArr = [];

document.querySelector('#beginQuestButton').addEventListener('click', function() {
    // Hide button
    let startGameButton = document.querySelector('#beginQuestButton');
...
function checkAnswer() {
    ...
    let answerSelectedText = questionsArr[randomNum].possibleAnswers[value];
    let correctAnswerText = questionsArr[randomNum].correctAnswer
...
function noCigar() {
    ...
    newNoCongratsSpan.textContent = `Sorry ... the answer is "${questionsArr[randomNum].correctAnswer}".`;
...
function displayFinalScreen() {
    ...
    let replayGame = document.createElement('button');
    replayGame.setAttribute('class', 'btn btn-warning');
    replayGame.setAttribute('id', 'replayGameButton');
    replayGame.textContent = 'Play Again';

    let results = document.querySelector('#answersArea');
    ...
    document.querySelector('#replayGameButton').addEventListener('click', function() {
        correctTally = 0;
        incorrectTally = 0;
